...
* 사이즈 크기 줄이는 방법
    - 레이어 단계 줄이기 (Layer를 생성하는 명령어는 Chaining 할 수 있는 경우 최대한 해주는 것이 좋다!)
    - 명령어 결합 하기 (RUN command1 && command2 )
    - 불필요한 데이터 복사 X (dockerignore 활용)
    - 불필요한 패키지 설치 X (개발용 패키지 설치 X, --no-install-recommends, Pipefile.lock과 같은 Lock 파일 등 사용)
    - .dockerignore 활용
    - 불필요한 파일 및 캐시 삭제 O (apt-get clean, rm -rf data 등 사용)
    - 압축파일 해제
    - Application 역할 분리 (하나의 역할을 가지게 구성)
    - 중간 이미지 하나로 합치기 (docker export & import)
    - Multi-stage 패턴 사용 : 빌드 등에는 필요하지만 최종 이미지에서는 필요 없는 환경을 제거할 수 있도록 단계를 나누어서 만드는 기법
        ```
