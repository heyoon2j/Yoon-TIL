...
* Domain
    - Broadcast Domain
    - Collision Domain
* Link-local Address (169.254.0.0/16) : Broadcast Domain 내에서 사용되는 임시 방편용 IP
* OSI 7 Layer
    - Physical : 물리 연결 통신
    - DataLink : MAC Address. 물리적인 통신 제어
    - Network : IP Address. 논리적인 통신에 대한 경로 제어
    - Transport : TCP/UDP. 논리적인 통신 제어 (신뢰성)
    - Session / Presentation / Application : 프로세스 간의 통신 관리, 데이터 형태 관리, 프로세스와 사용자 간의 상호작용 관리
* Casting
    - Unitcast : TCP / 1:1
    - Multicast : UDP / 1:n (Multicast Group 기준)
    - Broadcast : UDP / 1:n (Broadcast Domain 기준)
    - Anycast : IPv6 / 1:n (가장 가까운 장치)
* Protocol
    - ARP / RARP / DHCP
    - HTTP / FTP / SSH
* 3-Way handshake
    - SYN / SYN + ACK / ACK
    - LISTEN / SYN_SENT / SYN_RCV / ESTABLISHED / CLOSED
* 4-Way handshake
    - FIN / ACK, FIN / ACK
    - TIME_WAIT : 뒤늦게 도착하는 패킷이 있을 수 있으므로. 종료되면 Drop
* Timeout
...
* NAT
    - Source/Destination IP NAT
    - Source/Destination Port NAT
    - Loopback IP : 내부에서 처리하도록하는 IP
* 라우팅
    1) 경로 수집
    ...
    3) 경로 관리
    - 라우팅 기술
        - ECMP : Equal-cost multi-path routing. 동일한 속성 기준으로 여러 경로를 통해 전송되도록 허용하는 라우팅 전략
        - 비대칭 라우팅 : TGW Appliance Mode 설정 (NAT, Stateful 체크)
* VPN 터널링
    - Authentication (Pre-Shared Key or SSL 인증서)
    - Hash Key Algorithm ("Pre-Shared Key or SSL 인증서" 대한 무결성 검사 용도)
    - Encryption Algorithm (대칭키 암호화 알고리즘) -> 대칭키
    - DH Algorithm (키 교환 알고리즘) -> 대칭키 교환
    - 사용되는 기술
        - NAT-Traversal : VPN 장비가 NAT 뒤에 있는 경우, 추가 캡슐화가 필요
        - DPD(Dead peer detection) : 해당 VPN Connection이 죽었는지 확인하는 방식 (장비에서 지원이 되야됨)
* Cloud
    - VLAN : Layer 2
    - VXLAN : Layer 3
    - AZ affinity : 라우팅 장비는 기본적으로 전달받은 AZ로 트래픽을 전달하려고 한다.
...
* Subnet
    ...
    > 추가된 내용 : VPC, Subnet에서 설정하는 Zone의 의미는 해당 서브넷에서 생성 시에 설정한 Zone, 즉 해당 데이터센터에 생성하기 위한 메타데이터로 생각하면 된다. 그렇기 때문에 VPC 네트워크 서비스는 물리적으로 어디에 있는지 모른다!
* IPv6
...
* EIP
    ...
    - BYOIP : Bring Your own IP. 기존에 쓰고 있던 IP들을 가지고 올 수 있다! 대신 차단된 IP 같이 이상한 이력이 있으면 안된다. 이를 위해 우리는 ROA(Root Origin Authorization)
* Security Group
* Network ACL
</br>
...
* Transit Gateway
    ...
    - VPN ECMP support (Tunneling)
        ...
        - VPN의 Aggregation Throughput을 최대로 사용할 수 있다.
            1) 여러개의 VPN에 대해서 동일하게 라우팅
            2) VPN 내부 터널링 여러개
            > 터널의 각 BGP 속성 값을 동일학게 전파해줘야 된다는 의미이다!
...
* Direct Connect
    - Direct Connection (전용선)
        ...
        - 보안 : IPSec VPN 지원 가능. MACsec도 지원하나 생성 시에 설정 가능
            ...
            2) IPsec VPN : Layer 3 보안 (Site-to-Site VPN 통신 연결 시, Public VIF, Transit VIF 지원)
        ...
        - Connection Type
            ...
            2) Dedicated : 전용적으로 사용하는 Connection
                ...
                - VIF 구성시, VLAN을 원하는 범위에서 설정 가능할 것을 보임 (왜? It's mine. 내꺼니까)
        - VIF : Connection을 VLAN으로 분리시킨 가상 인터페이스
            1) Public : DX 전용 회선을 통해 Public AWS 서비스(S3, DynamoDB, CloudFront)에 연결. 실제 인터넷을 통해 가지 않는다!!)
                - 1000 prefix
        ...
        - LAG (Link Aggregation Group) : 링크 집계 그룹. 여러 개의 링크(Connection)를 하나의 논리적 연결로 취급.
            ...
            > 기능은 여러 경로에 대해 분산처리하는 것이다. 단지 차이점은 다음과 같다. ECMP는 '다수의 경로를 개별적으로 관리'하면서 분산처리하는 것이고, LAG는 다수의 경로를 '하나의 논리적인 경로로 관리'하면서 내부적으로 분산처리한다.
            
            > 그렇기때문에 On-prem에서도 하나의 장치로 바라보게 된다.
        - Resiliency (복원력)
...
* IPAM : IP Management
    ...
    - Global / Region / Environment(개발/운영) / VPC - Level까지 사용가능 
    - 관리자 계정이 모든 계정을 관리
    - Tier
        - Free : 공용 IP 관리만 가능
...
* VPC Reachability Analyzer
    - VPC 내에서 특정 리소스 간의 네트워크 경로가 정상적으로 설정되어 있는지 확인하는 도구
...
* Network Access Analyzer
    ...
    > VPC Reachability Analyzer vs Network Access Analyzer : RA는 특정 대상에 대한 것이고, NAA는 접근 권한 등의 보안에 대해 확인하는 용도이다. 그렇기 때문에 네트워크 & 보안 거버넌스 분석을 위해서는 NAA를 위한 용도
* Tool
...
- Preventive service : 위험이나 문제 발생 이전에 이를 방지하기 위해 설계된 서비스 (예방 서비스)
    - 방화벽(침입 차단 시스템) / MFA / 패치 관리 / 데이터 암호화
- Detective service : 위험이나 문제가 발생한 후 이를 식별(분석)하고 대응하기 위해 설계된 서비스 (탐지 서비스)
    - IDS(침입 탐지 시스템) / 로그 모니터링 / SIEM(보안 정보 및 이벤트 관리) / 보안 카메라
> Detective service는 즉각적인 처리보다는 말그대로 분석하고 경고 및 정보를 제공하는데 초점이 맞춰져있다. 그렇기 때문에 IPS는 예방, 즉 처리 기능이 들어가 있기때문에 Preventive service에 가깝다. 
...
* Network Firewall : Layer 3-7 - AWS Network Appliance
    ...
    - 특징
        ...
        - Statefule/L4 rule
        - IPS-IDS/L7 rule ('HTTP 헤더 필터링'과 같은 일부 기능 지원)
        - FQDN Filtering
        - Protocol detection
