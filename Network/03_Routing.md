...
- eBGP
- 다른 쪽 AS 라우터에 어느 쪽으로 들어오는 것이 좋은지 전달
...
> Local Preference vs MED 차이는 다른 AS로 넘어갈때 LOCAL_PREF는 전체적인 경로를 기준으로 본다면(A -> B -> C), MED는 LOCAL_PREF에서 결정된 경로에서 각 다음 네트워크를 넘어갈때 경로를 기준으로 본다(A1 -> B, A2 -> B, B -> C1, B -> C2 등).
